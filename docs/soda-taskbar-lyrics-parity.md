...
## 15. CI 由红转绿：整行唱满后的高亮尾巴 + README 重写

- **需求（用户原话）**：「那个Releases的贡献者是谁？然后图中问题能解决么，给README.md重写。」附图 `image.png` 117×40 px（sha256 `7b56b5c6dd36170261aaa1abb7067ee2c468ab9167b3a81c6b0a8c4a56519667`）。本机没有视觉通道，只能用 PowerShell + `System.Drawing` 的 `LockBits` / `Marshal.Copy` 解码：纯白底 2146 px + 红色圆角块（红阶 `#D53645 #D63847 #D33341 #D43543 #D73A49 #CD2734`）+ 白色反白文字，与 `badge.svg` 的 `state-fill` 渐变 `#D73A49 → #CB2431` 完全一致 ⇒ **附图就是 README 第 3 行那个 GitHub Actions 徽章（failing）**。
- **贡献者**：Release 作者 = 资产 uploader = **`DoingStone`**（id `136030597`），本地 `user.name` 与它相同；最近四次提交全是同一身份 ⇒ 没有第三方参与，Release 是仓库自己的账号发的。
- **CI 日志取证配方（可复用）**：`GET /repos/{repo}/actions/runs?head_sha=<sha>&per_page=1` 取 run id → `GET /actions/runs/{id}/jobs` 取 job id 与逐步结果 → `curl.exe -sSL -o <file> -H "Authorization: Bearer <token>" -H "Accept: application/vnd.github+json" https://api.github.com/repos/{repo}/actions/jobs/{job_id}/logs`。注意这份日志是**纯文本**（首字节 `239,187` = UTF-8 BOM），不是 zip，别拿 `ZipFile::ExtractToDirectory` 去解（会报「找不到中央目录结尾记录」）。
- **失败点**：仓库 12 次运行全是 failure；最近一次 run `35825558254`（sha `446ecb0`）job `107066374509`，第 1–6 步成功（`0 Error(s)`、引擎 `self-test: 101 passed, 0 failed`），**第 7 步 `Settings self-tests` 失败**，第 8/9 步被跳过。原文逐字：
  - `FAIL  fully swept: highlight covers the whole line  (highlight=353 text=[246..356])`
  - `word sweep: 4 passed, 1 failed`；`sweep follows position: 7 passed, 0 failed`；`settings self-test: 28 passed, 1 failed`
  - `throw "settings self-tests failed (exit $($p.ExitCode))"` ⇒ `settings self-tests failed (exit 1)` ⇒ `##[error]Process completed with exit code 1.`
- **根因（两条，都在 `Controls\KaraokeLine.cs` 的 highlight pass，都是本会话 P2/P3 改动留下的）**：
  1. P3a 的柔边**在整行唱满后仍在淡出尾巴**：`fade = Math.Min(FontSizeValue * HighlightFadeRatio, sungWidth)`（`HighlightFadeRatio = 0.45`，16 px 字号约 7.2 DIP）⇒ 最后几个字比其余部分暗，量到的高亮右边界比文字墨迹少约 3 px。
  2. 裁剪矩形**正好停在排版宽度**：`new Rect(left, 0, sungWidth, ActualHeight)`，而**字形墨迹会超出它的 advance 宽度** ⇒ 最后一个字的最右一列永远不会被高亮。
- **修法**（`KaraokeLine.cs`）：新增 `private const double HighlightEndSlack = 2.0;`（紧跟 `HighlightFadeRatio` 之后），highlight pass 改成
  `var complete = sungWidth >= main.Width - 0.5;` →
  `var clip = new RectangleGeometry(new Rect(left, 0, complete ? sungWidth + HighlightEndSlack : sungWidth, ActualHeight));` →
  `var fade = complete ? 0.0 : Math.Min(FontSizeValue * HighlightFadeRatio, sungWidth);`
  依据是 `MeasureSungWidth(FormattedText main, double progress)` 里 `if (progress >= 1) return main.Width;` ⇒ 唱满时 `complete` 必为真。**行进中仍然淡出、行中段裁剪仍停在高亮边缘**（那条路径一个字没改）。
- **自检也修了**（`Configuration\SettingsSelfTest.cs` 的 `VerifyWordSweep`）：分类器原来是「饱和蓝」`if (b > 150 && b > r + 40 && g > r)`，而字形最后一两列只是**很淡的抗锯齿斜坡**，于是它把唱满的行报成「比自己的文字短 1–2 px」。改为「任何蓝色染色」`if (b > r + 6)`（白底上 DeepSkyBlue 每 5% 覆盖把蓝抬到红之上约 13，而未唱的白/灰是 `b == r`）；断言本身保留 **2 px 容差**并写明理由（最后一列是亚像素斜坡，跨机器分类可能有别；而真正的回归——淡出的尾巴或整段没高亮——远大于 2 px）。
- **数值演进（本地 Release 版 `--selftest` 复现）**：CI 修前 `highlight=353`（FAIL）→ 只修 fade `354`（恰好卡在 `fEnd−2` 阈值上）→ 再加 2 DIP 裁剪空余仍 `354` → 分类器改成蓝色染色 `355` ⇒ 留出 1 px 余量。最终 `settings self-test: 29 passed, 0 failed`（原 28/1）、`selftest exit: 0`、`word sweep: 5 passed, 0 failed`。
- **结果**：提交 **`4660131`**「Highlight: never fade the tail of a line that has finished sweeping」（2 files, +33/−4），推送 `446ecb0..4660131`；CI run **`35849300995`** 全部 9 个步骤 success（含原来失败的第 7 步与第 8/9 步）；徽章 `curl.exe -s -m 25 '…/badge.svg?branch=main'` ⇒ `<title>CI - passing</title>`，**附图上那个红徽章问题已解决**。
- **回归**：`tools\acceptance.ps1` 19/19（引擎 101、设置自检 29、SMTC `score=95 lines=54 source=QqMusic`、叠加窗 575×58、381 种颜色）；`tools\verify-cluster-modes.ps1` 全 PASS。`tools\verify-hover-shift.ps1` 首次跑出 **2 项 FAIL**（`rest centred: ink centre 253.3 vs band centre 287.5 (off 34.2 px, ink 3674 px)`、`hover words: left edge 10 px … ink centre 256.6 vs 287.5`），同一构建**换一行歌词复跑却全 PASS**（`ink centre 284.5 vs 287.5`、`left edge 323 px (room starts at 320)`、`ink width 238 px at rest vs 238 px on hover`）⇒ 判定为**套件的状态依赖脆弱点，不是本次回归**：该脚本用「与底色亮度差 > 40」判定墨迹，而未唱字色 `#FFE8E8E8`（luma 232）贴近浅色任务栏底色（众数 220，差 12）⇒ 它只统计到**已唱**那一截，中线随扫描位置漂移。那次 FAIL 的墨迹 bbox 是 `left=99 px`（= 79.2 DIP，正好等于 460 DIP 条带居中所需的 80 DIP），几何上文字本来就是居中的。**该脆弱点未修**，已写进 README 的验证状态表备注（修法：按基色/高亮色做颜色匹配，而不是单一亮度阈值）。
- **README 重写**（`README.md`，463 → 510 行，+168/−121）：新增顶部徽章行（CI / Release / License / .NET 8）与「目录」；新增「下载与安装」独立成节（Releases → 解压 → .NET 8 Desktop Runtime → 双击即开设置窗，并写明 zip 不含字体）；新增「功能一览」表；技术要点补齐第 5 小节「渲染循环」（8 ms 定时器 + 12 ms 下限护栏、实测 56–63 fps、逐像素透明强制软件渲染的代价）与逐字高亮里的不变量（行进中柔边、唱满关渐变 + 2 DIP 空余，即 CI 那条自检守的东西）；配置表按 `AppSettings.cs` 实测默认值校正并补 `Height` / `ShowWhenPaused` / `HideWhenNoLyrics`；新增**调试用环境变量表**（`TBL_SETTINGS` / `TBL_DIAG` / `TBL_COMPOSITE`）；「验证状态」改成「持续集成」+「本机验收」两段并挂上绿 run 链接，工具表补 `verify-toggle-disc.ps1` 一行且**明写两条脚本脆弱点**（hover-shift 的掩码、toggle-disc 需要设置窗在前台）；故障排查补「拖不动歌词条」；更新日志加「未发布」段记录本次高亮修复。
