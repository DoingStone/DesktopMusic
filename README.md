...
- **翻译行**：有翻译时在原文下方联动显示
- **下一行预览**：提前显示即将演唱的歌词
- **点击穿透**：锁定状态下完全不挡任务栏操作（悬停时仍会显示播放按键）
- **可拖动 / 可悬浮**：解锁后可直接拖动；往任务栏外拖就变成悬浮小条，能放到屏幕任意位置，拖动时自动吸附屏幕边缘与居中，拖回任务栏即重新入坞，位置自动记忆
...
| 移动位置 | 设置 → 取消「锁定位置」→ 直接拖动（含上下方向）→ 再锁定；或用设置中的方向微调按钮 |
...
往任务栏外拖（离开任务栏那条带子）就变成**悬浮小条**，可以放到屏幕任意位置；拖出屏幕会被自动夹住，不会丢。
拖动时默认**吸附对齐**（屏幕左/右/上/下边缘、水平与垂直居中、任务栏那一行），阈值 12 DIP，松手前就能看到对齐结果；不想要可以在设置里关掉。

拖回任务栏上松手即**重新入坞**（回到原来的水平位置），也可以直接点「尺寸与位置 → 吸附回任务栏」，或托盘右键 →「恢复默认位置」。
